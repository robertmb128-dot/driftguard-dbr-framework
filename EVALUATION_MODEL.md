...
## 6. Score Domain Bounds

After calculation, enforce:

0.00 ≤ Framework Score ≤ 1.00

Any value outside this range indicates an adapter error.
...
## 8. Missing Control Handling

Evaluation order:

1. Check if an approved exception exists  
2. If yes → ComplianceValue = 1.0  
3. If no → evaluate control normally  
4. If evaluation fails → ComplianceValue = 0.0  
...
- `status = Active`  
- `expiration_date ≥ evaluation_date (UTC)`  
...
**Evaluation date must be defined in UTC to guarantee determinism.**
